...
class PayService
{
	...
	public static function jsapiPay($data,$config){
		if(empty($data['body'])) throw new ValidateException ('交易标题不能为空');
		if(empty($data['out_trade_no'])) throw new ValidateException ('交易订单号不能为空');
		if(empty($data['total_fee'])) throw new ValidateException ('交易金额不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['total_fee'])){
			throw new ValidateException ('交易金额格式错误');
		}
		if(empty($data['notify_url'])) throw new ValidateException ('回调地址不能为空');
		if(empty($data['openid'])) throw new ValidateException ('用户openid不能为空');
		...
		try{
			...
			throw new ValidateException ('微信支付失败');
		}
	 ...
	 */
	public static function nativePay($data){
		if(empty($data['body'])) throw new ValidateException ('交易标题不能为空');
		if(empty($data['out_trade_no'])) throw new ValidateException ('交易订单号不能为空');
		if(empty($data['total_fee'])) throw new ValidateException ('交易金额不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['total_fee'])){
	...
	public static function payToUserBlance($data,$config){
		if(empty($data['openid'])) throw new ValidateException('用户openid不能为空');
		if(empty($data['re_user_name'])) throw new ValidateException('用户真实姓名不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['amount'])){
			throw new ValidateException('付款金额格式错误');
		}
		if(empty($data['desc'])) throw new ValidateException('付款说明不能为空');
	...
	public static function payToUserCard($data){
		if(empty($data['enc_bank_no'])) throw new ValidateException('银行卡号不能为空');
		if(empty($data['enc_true_name'])) throw new ValidateException('用户真实姓名不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['amount'])){
			throw new ValidateException('付款金额格式错误');
		}
		if(empty($data['bank_code'])) throw new ValidateException('银行编号不能为空');
		if(empty($data['desc'])) throw new ValidateException('付款说明不能为空');
	...
	public static function sendNormalRedpack($data){
		if(empty($data['send_name'])) throw new ValidateException('商户名称不能为空');
		if(empty($data['re_openid'])) throw new ValidateException('用户openid不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['total_amount'])){
			throw new ValidateException('红包金额格式错误');
		}
		if(empty($data['wishing'])) throw new ValidateException('祝福语不能为空');
		if(empty($data['act_name'])) throw new ValidateException('活动名称不能为空');
		if(empty($data['remark'])) throw new ValidateException('活动备注不能为空');
	 ...
	 * @return array   返回订单详情
	 */
	public static function sendGroupRedpack($data,$config){
		if(empty($data['send_name'])) throw new ValidateException('商户名称不能为空');
		if(empty($data['re_openid'])) throw new ValidateException('用户openid不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['total_amount'])){
			throw new ValidateException('红包金额格式错误');
		}
		if($data['total_amount'] < 3) throw new ValidateException('裂变红包码最少3元');
		if($data['total_num'] < 3) throw new ValidateException('裂变次数最少三次');
		if(empty($data['wishing'])) throw new ValidateException('祝福语不能为空');
		if(empty($data['act_name'])) throw new ValidateException('活动名称不能为空');
		if(empty($data['remark'])) throw new ValidateException('活动备注不能为空');
	 ...
	 * @return array   返回订单详情
	 */
	public static function payQuery($out_trade_no,$config){
		if(empty($out_trade_no)) throw new ValidateException('交易订单号不能为空');
	 ...
	 * @return array   返回退款详情
	 */
	public static function refund($data,$config){
		if(empty($data['out_trade_no'])) throw new ValidateException('交易订单号不能为空');
		if(empty($data['total_fee'])) throw new ValidateException('订单金额不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['total_fee'])){
			throw new \Exception('订单金额格式错误');
		}
		if(empty($data['refund_fee'])) throw new ValidateException('退款金额不能为空');
		if(!preg_match('/(^[1-9]([0-9]+)?(\.[0-9]{1,2})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/',$data['refund_fee'])){
			throw new ValidateException('退款金额格式错误');
		}
		if(empty($data['desc'])) throw new ValidateException('退款说明不能为空');
	...
	public static function refundQuery($refundId){
		if(empty($refundId)) throw new ValidateException('退款交易号不能为空');
		$payment = Factory::payment(array_merge(config('my.official_accounts'),config('my.wechart_pay')));
		try{
